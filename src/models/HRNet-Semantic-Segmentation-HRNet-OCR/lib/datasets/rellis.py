...
class Rellis(BaseDataset):
    ...
    def multi_scale_inference(self, config, model, image, scales=[1], flip=False):
        ...
        image = image.numpy()[0].transpose((1, 2, 0)).copy()# convert to desired format
        stride_h = np.int(self.crop_size[0] * 1.0) # uses the entire size of the output image for the stride
        stride_w = np.int(self.crop_size[1] * 1.0)
        final_pred = torch.zeros([1, self.num_classes,
                                  ori_height, ori_width]).cuda()
        # for each scale in the multi-scale inference
        for scale in scales:
            ...
            if scale <= 1.0:  # uses this one, single scale being used by Rellis implementation 
                new_img = new_img.transpose((2, 0, 1))
                new_img = np.expand_dims(new_img, axis=0)
                new_img = torch.from_numpy(new_img)
                preds = self.inference(config, model, new_img, flip)
                preds = preds[:, :, 0:height, 0:width]
            #  ------ Not being used in native implementation -> single scale ONLY
            else:
