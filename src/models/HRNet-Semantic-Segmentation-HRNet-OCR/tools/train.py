...
def parse_args():
    ...
    parser.add_argument("--local_rank", type=int, default=-1) # defaults to not being a distributed process (this is desired)
...
def main():
    ...
    # # Generates a summary list for the model
    ...
    # prepare data
    crop_size = (config.TRAIN.IMAGE_SIZE[1], config.TRAIN.IMAGE_SIZE[0]) #crops the source image into smaller images
    ...
    end_epoch = config.TRAIN.END_EPOCH + config.TRAIN.EXTRA_EPOCH # trains for both the set epoch and the extra epochs present there 
    ...
    # Obtain the summary information regarding the model
    #NOTE: torchsummary will ONLY work if the loss distribution functions in the FullModel class are 
    #commented out of the program. Make sure to uncomment before training the network.
    
    # import torchsummary
    # torchsummary.summary(model, (3,1200,1920)) 
