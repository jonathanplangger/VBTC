...
class HighResolutionNet(nn.Module):
    ...
    def init_weights(self, pretrained='',):
        ...
        if os.path.isfile(pretrained):
            ...
            # logger.info('=> loading pretrained model {}'.format(pretrained)) # removed the large logging blob for the pre-trained model params
            model_dict = self.state_dict()
            pretrained_dict = {k.replace('last_layer', 'aux_head').replace('model.', ''): v for k, v in pretrained_dict.items()}  
            # Remove the printing of parameters -> not relevant as of right now 
...
def get_seg_model(cfg, **kwargs):
    ...
    # model.init_weights(cfg.MODEL.PRETRAINED) # not using any pre-training for the model. 
